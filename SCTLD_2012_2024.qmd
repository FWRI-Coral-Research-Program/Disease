...
fk_sctld <- read_excel("C:/Users/Madison.Enda/Desktop/Microsoft_Access_Files/z_Colonies.xlsx")

# Use read_excel() to load in the files on southeastern florida sctld
sf_sctld <- read_excel("C:/Users/Madison.Enda/Desktop/Microsoft_Access_Files/SEFL_Colonies.xlsx")
```
...
## Create a UNK_DIS & SumOfAllDisease column

```{r}
# Create a list of disease and condition columns
condition_cols <- c("BLH", "PBL", "PAL", "AnyBleaching", "BLH+PBL", "BBD", "WBD", "WPX", "WPL", "DSD", "DOB", "RTL", "RBD", "YBD", "AnyDisease", "SKA", "CLN", "CDL", "PRD", "DAM", "OUT")


# Create our UNK_DIS column
fk_sctld_filtered <- fk_sctld_filtered %>%
  mutate(
    UNK_DIS = if_else(
      `%RECENT` > 0 &
      UNK > 0 &
      rowSums(across(all_of(condition_cols)) == 1) == 0,
      1,
      0
    )
  )
...
```{r}
# Create our SumofAllDisease column
fk_sctld_filtered <- fk_sctld_filtered %>%
  mutate(SumOfAllDisease = rowSums(
    select(., DSD, DOB, WPL, WPX, BBD, WBD, RBD, YBD, RTL, UNK_DIS), # replace with your disease columns
    na.rm = TRUE
  ))
```
...
annual_sctld_lk <- lower_sctld %>%
  ...
  group_by(SampleYear) %>%
  summarise(
    Counts_SCTLD = sum(DOB, na.rm = TRUE),
    Counts_AllDisease = sum(SumOfAllDisease, na.rm = TRUE),
    Total_Coral_Counts = n(),
    SCTLD_percent = (Counts_SCTLD / Total_Coral_Counts) * 100,
    Disease_percent = (Counts_AllDisease / Total_Coral_Counts) * 100,
    .groups = "drop"
  )
```
...
```{r}
# Create a list of disease and condition columns
condition_cols <- c("BLH", "PBL", "PAL", "AnyBleaching", "BLH+PBL", "BBD", "WBD", "WPX", "WPL", "DSD", "RTL", "RBD", "YBD", "AnyDisease", "SKA", "CLN", "CDL", "PRD", "DAM", "OUT")
...
sf_sctld_filtered <- sf_sctld_filtered %>%
  mutate(SumOfAllDisease = rowSums(
    select(., DSD, WPL, WPX, BBD, WBD, RBD, YBD, RTL, UNK_DIS), # replace with your disease columns
    na.rm = TRUE
  ))
```
...
annual_sctld_sf <- sf_sctld_filtered %>%
  group_by(SampleYear) %>%
  summarise(
    Counts_SCTLD = sum(WPL, na.rm = TRUE),
    Counts_AllDisease = sum(SumOfAllDisease, na.rm = TRUE),
    Total_Coral_Counts = n(),
    SCTLD_percent = (Counts_SCTLD / Total_Coral_Counts) * 100,
    Disease_percent = (Counts_AllDisease / Total_Coral_Counts) * 100,
...
# Rename columns 
fk_sctld_filtered <- fk_sctld_filtered %>%
  rename(fk_disease = SumOfAllDisease,
         White_Plauge = WPL)
...
# Create a list of disease and condition columns
condition_cols <- c("BLH", "PBL", "PAL", "AnyBleaching", "BLH+PBL", "BBD", "WBD", "WPX", "WPL", "White_Plauge", "DSD", "RTL", "RBD", "YBD", "AnyDisease", "SKA", "CLN", "CDL", "PRD", "DAM", "DOB", "OUT")
...
# Create the SumOfAllDisease column again
combined_species<- combined_species %>%
  mutate(SumOfAllDisease = rowSums(
    select(., DSD, WPL, WPX, BBD, WBD, DOB, White_Plauge, RBD, YBD, RTL, UNK_DIS), # replace with your disease columns
    na.rm = TRUE
  ))
...
annual_sctld_species <- combined_species %>%
  ...
  summarise(
    ...
    Disease_percent = if_else(
      Total_Coral_Counts > 0,
      (Counts_AllDisease / Total_Coral_Counts) * 100,
      NA_real_
    ),
    .groups = "drop"
  )
